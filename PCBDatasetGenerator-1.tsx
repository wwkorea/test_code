...
import {
  PCB_W, PCB_H, SPEC,
  Point, AnomalyType, PCBComponent, Trace, TraceCut, PCBTemplate, DefectRegion,
...
interface ApplyResult {
  ...
  regions: DefectRegion[]; // 마스크용 결함 영역 (정상이면 빈 배열)
}

// 부품 종류별 대략적인 바운딩박스 반각 (마스크 rect 크기 계산용)
function compHalfExtent(c: PCBComponent): { hw: number; hh: number } {
  switch (c.type) {
    case "ic":        return { hw: c.w / 2 + SPEC.IC_PAD_REACH, hh: c.h / 2 + 4 };
    case "resistor":  return { hw: SPEC.RES_DX + 6, hh: 9 };
    case "cap":       return { hw: SPEC.CAP_DX + 6, hh: 12 };
    case "led":       return { hw: SPEC.LED_DX + 5, hh: 9 };
    case "connector": return { hw: c.pins * SPEC.CONN_PITCH / 2 + 4, hh: 12 };
  }
}
...
function applyNoiseAndDefects(
    ...
    switch (a) {
      case "open": {
        ...
        if (candidates.length > 0) {
          ...
          const at = Math.floor(rand() * (t.points.length - 1));
          t.cut = { at, gap: 5 + rand() * 7 } as TraceCut;
          // gap 중심 = 끊긴 세그먼트의 중점
          const a0 = t.points[at], b0 = t.points[at + 1];
          regions.push({
            kind: "circle",
            x: (a0[0] + b0[0]) / 2, y: (a0[1] + b0[1]) / 2,
            r: 7, anomaly: "open",
          });
        }
        labels.push("단선");
      ...
      case "bridge": {
        ...
        const len = 7 + rand() * 8;
        const bw = 2.5 + rand() * 2;
        const bx1 = mx - Math.cos(ang) * len / 2, by1 = my - Math.sin(ang) * len / 2;
        const bx2 = mx + Math.cos(ang) * len / 2, by2 = my + Math.sin(ang) * len / 2;
        traces.push({
        ...
        regions.push({ kind: "line", x1: bx1, y1: by1, x2: bx2, y2: by2, r: bw / 2 + 2, anomaly: "bridge" });
        labels.push("브릿지");
        break;
      }
      case "misalign": {
        ...
        if (candidates.length > 0) {
          ...
          const { hw, hh } = compHalfExtent(comps[i]);
          regions.push({
            kind: "rect",
            x: comps[i].x - hw, y: comps[i].y - hh,
            w: hw * 2, h: hh * 2, anomaly: "misalign",
          });
        }
        labels.push("오정렬");
        break;
      ...
      case "polarity": {
        ...
        if (targets.length > 0) {
          ...
          tgt.reversed = true;
          const { hw, hh } = compHalfExtent(tgt);
          regions.push({
            kind: "rect",
            x: tgt.x - hw, y: tgt.y - hh,
            w: hw * 2, h: hh * 2, anomaly: "polarity",
          });
        }
        labels.push("극성반전");
        break;
      ...
      case "scratch": {
        ...
        const ang = rand() * Math.PI;
        const sx1 = cx - Math.cos(ang) * len / 2, sy1 = cy - Math.sin(ang) * len / 2;
        const sx2 = cx + Math.cos(ang) * len / 2, sy2 = cy + Math.sin(ang) * len / 2;
        traces.push({
          isScratch: true, points: [], w: 1.5, net: "SCRATCH",
          x1: sx1, y1: sy1, x2: sx2, y2: sy2,
        });
        regions.push({ kind: "line", x1: sx1, y1: sy1, x2: sx2, y2: sy2, r: 3, anomaly: "scratch" });
        labels.push("스크래치");
        break;
      }
      case "solder_cold": {
        ...
        if (candidates.length > 0) {
          ...
          const { hw } = compHalfExtent(comps[i]);
          // 냉납은 패드 부위 결함 → 부품 전체를 덮는 원으로 표기
          regions.push({
            kind: "circle",
            x: comps[i].x, y: comps[i].y,
            r: hw, anomaly: "solder_cold",
          });
        }
        labels.push("냉납");
        break;
  ...
  return { comps, traces, labels, regions };
}

// ─── 마스크 렌더러 (U-Net 학습용) ─────────────────────────────────────────────
//  배경=검정, 결함 영역=흰색. regions가 비면 전체 검정(정상 마스크).
...
export default function PCBDatasetGenerator(): JSX.Element {
  ...
  const [apiUrl, setApiUrl]               = useState<string>("http://localhost:8000/upload");
  ...
  const render = useCallback((): void => {
    ...
    if (!ctx) return;
    const { comps, traces, regions } = applyNoiseAndDefects(currentTemplate, activePreviewDefects, seed);
    drawPCB(ctx, comps, traces, showGrid, `PCB-${currentTemplate.id} v4`);
    // 마스크도 같은 결과로 렌더
    const mctx = maskRef.current?.getContext("2d");
    if (mctx) drawMask(mctx, regions);
  ...
  const downloadSingle = (): void => {
    ...
    const base = `pcb_${currentTemplate.id}_${defectStr}_seed${seed}`;
    // 이미지
  ...
  const generateBatch = async (): Promise<void> => {
    ...
    // 이미지/마스크 파일명을 쌍으로 기록 (동일 base + _mask)
    let csv = "filename,mask_filename,template,anomaly_types,anomaly_labels,seed\n";

    const off = document.createElement("canvas");
    off.width = PCB_W; off.height = PCB_H;
    const offCtx = off.getContext("2d");
    const offMask = document.createElement("canvas");
    offMask.width = PCB_W; offMask.height = PCB_H;
    const offMaskCtx = offMask.getContext("2d");
    if (!offCtx || !offMaskCtx) return;

    for (const tpl of activeTemplates) {
      for (const cls of classes) {
        for (let i = 0; i < perCell; i++) {
          const s = Math.floor(Math.random() * 999999);
          const { comps, traces, labels, regions } = applyNoiseAndDefects(tpl, cls, s);
          drawPCB(offCtx, comps, traces, false, `PCB-${tpl.id} v4`);
          drawMask(offMaskCtx, regions);

          const clsStr = cls.length > 0 ? cls.join("+") : "normal";
          const base = `pcb_${tpl.id}_${clsStr}_${String(i).padStart(4, "0")}_s${s}`;
          const fname = `${base}.png`;
          const mname = `${base}_mask.png`;
          csv += `${fname},${mname},${tpl.id},${clsStr},"${labels.join("+")}",${s}\n`;

          if (sendToApi) {
            // FastAPI 전송: 이미지 + 마스크를 한 번에 보냄
            const imgBlob = await new Promise<Blob | null>((res) => off.toBlob(res, "image/png"));
            const maskBlob = await new Promise<Blob | null>((res) => offMask.toBlob(res, "image/png"));
            if (imgBlob && maskBlob) {
              const fd = new FormData();
              fd.append("image", imgBlob, fname);
              fd.append("mask", maskBlob, mname);
              fd.append("template", tpl.id);
              fd.append("anomaly_types", clsStr);
              fd.append("seed", String(s));
              try {
                await fetch(apiUrl, { method: "POST", body: fd });
              } catch {
                // 네트워크 실패는 로그에만 남기고 계속
                newLog.push(`⚠ 전송 실패: ${fname}`);
              }
            }
          } else if (total < 5) {
            // 다운로드 모드: 브라우저 제한으로 처음 5쌍만
            const link = document.createElement("a");
            link.download = fname; link.href = off.toDataURL("image/png"); link.click();
            await new Promise<void>((r) => setTimeout(r, 60));
            const mlink = document.createElement("a");
            mlink.download = mname; mlink.href = offMask.toDataURL("image/png"); mlink.click();
            await new Promise<void>((r) => setTimeout(r, 60));
          }
        ...
        <div style={{ display: "flex", gap: 20, flexWrap: "wrap" }}>
          ...
          <div style={{ flex: "0 0 auto" }}>
            <div style={panelStyle}>
              ...
              <div style={{ display: "flex", gap: 10, alignItems: "flex-start" }}>
                <div>
                  <div style={{ fontSize: 10, color: "#8b949e", marginBottom: 4 }}>이미지</div>
                  <canvas
                    ref={previewRef}
                    width={PCB_W}
                    height={PCB_H}
                ...
                <div style={{ display: showMask ? "block" : "none" }}>
                  <div style={{ fontSize: 10, color: "#8b949e", marginBottom: 4 }}>마스크 (U-Net 정답)</div>
                  <canvas
                    ref={maskRef}
                    width={PCB_W}
                    height={PCB_H}
                    style={{
                      display: "block", border: "1px solid #30363d", borderRadius: 4,
                      width: PCB_W, height: PCB_H,
                    }}
                  />
                </div>
              </div>
              ...
              <div style={{ marginTop: 12, display: "flex", gap: 8, flexWrap: "wrap" }}>
                ...
                <button onClick={downloadSingle} style={btnStyle("#1f6feb")}>⬇ 이미지+마스크 저장</button>
          ...
          <div style={{ flex: 1, minWidth: 280 }}>
            ...
            <div style={panelStyle}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 6, marginBottom: 12, padding: "10px", background: "#0d1117", borderRadius: 6 }}>
                <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 11, cursor: "pointer", color: sendToApi ? "#58a6ff" : "#8b949e" }}>
                  <input type="checkbox" checked={sendToApi} onChange={(e) => setSendToApi(e.target.checked)} />
                  FastAPI로 전송 (이미지 + 마스크)
                </label>
                {sendToApi && (
                  <input
                    ...
                    placeholder="http://localhost:8000/upload"
                    style={{
                      background: "#161b22", border: "1px solid #30363d", color: "#e6edf3",
                      borderRadius: 4, padding: "4px 8px", fontFamily: "monospace", fontSize: 11,
                    }}
                  />
                )}
                <div style={{ fontSize: 9, color: "#555" }}>
                  {sendToApi
                    ? "전송 모드: 전체 이미지+마스크를 POST합니다 (다운로드 안 함)."
                    : "다운로드 모드: 처음 5쌍만 브라우저로 저장됩니다."}
                </div>
              </div>
